...
#include "ss_dsi_mdnie_S6TUUM1_AMSA24VU04.h"
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_hbm(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->vrr.adjusted_refresh_rate == 60) { /* 60hz : from hbm_60hz_gamma_table in ~.h */
		hbm_tb_idx = hbm_60hz_brightness_tb_idx[vdd->br_info.common_br.bl_level];
		cmd_idx = ss_get_cmd_idx(hbm_cmds, 0x7B, 0x67);
		hbm_cmds->cmds[cmd_idx].ss_txbuf[1] = hbm_60hz_brightness_tb[hbm_tb_idx][hbm_bl_set_idx++];
		hbm_cmds->cmds[cmd_idx].ss_txbuf[2] = hbm_60hz_brightness_tb[hbm_tb_idx][hbm_bl_set_idx++];
		cmd_idx = ss_get_cmd_idx(hbm_cmds, 0x82, 0x67);
		hbm_cmds->cmds[cmd_idx].ss_txbuf[1] = hbm_60hz_brightness_tb[hbm_tb_idx][hbm_bl_set_idx++];
...
static struct dsi_panel_cmd_set *ss_irc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->br_info.common_br.irc_mode == IRC_MODERATO_MODE)
		irc_on_cmds->cmds->ss_txbuf[2] = IRC_MODERATO_MODE_VAL;
	else if (vdd->br_info.common_br.irc_mode == IRC_FLAT_GAMMA_MODE)
		irc_on_cmds->cmds->ss_txbuf[2] = IRC_FLAT_GAMMA_MODE_VAL;
	else
...
{
	LCD_INFO(vdd, "S6TUUM1_AMSA24VU01 : ++\n");
